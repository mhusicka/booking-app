...
// KONFIGURACE (Získejte z Renderu)
...
async function finalizeReservation(reservation) {
    ...
    // Zde by normálně následovalo odeslání PDF faktury a Emailu (viz předchozí kódy)
    return reservation;
}
...
app.get("/admin/reservations", checkAdmin, async (req, res) => {
    const list = await Reservation.find().sort({ created: -1 });
    res.json(list);
});
